...
ggplot(violencia_mes_todo) +
  ...
  labs(
    title = "Violencia Familiar Meses comparables",
    subtitle = "Porcentaje de del total de casos reportados por mes y año en toda la Republica",
    x = "",
...
ggplot(violencia_mes_todo) +
  ...
  scale_y_continuous(labels = scales::percent) +
  labs(
    title = "Porcentaje Violencia Familiar",
    subtitle = "Comparativa meses con datos en toda la republica",
    x = "",
    y = "Porcentaje del total de casos",
    color = ""
  ) +
...
violencia_mes_entidad_varios <- DatosMesEstadoAgrupados(
  datos_violencia, entidad = "Todas", resaltar.tipo = c(
    "Violencia familiar", "Abuso sexual", "Violencia de género en todas sus modalidades distinta a la violencia familiar"
  )
)
violencia_mes_entidad_varios <- violencia_mes_entidad_varios %>% 
  filter(Tipo !=  "Otros tipos de Violencia") %>% 
  mutate(Tipo = ifelse(
    Tipo == "Violencia de género en todas sus modalidades distinta a la violencia familiar",
...
ggplot(violencia_mes_entidad_varios) +
  ...
  scale_y_continuous(labels = scales::percent) +
  labs(
    title = "Porcentaje Violencia Familiar, Acoso Sexual y modalidades distintas a violencia familiar",
    subtitle = "Comparativa meses con datos en toda la republica",
    x = "",
    y = "Porcentaje del total de casos",
    color = ""
...
violencia_tags_inicio <- ranking_tipo_violencia_republica %>%
  ...
  mutate(anyo = as.numeric(anyo) - 0.25)
violencia_tags_final <- ranking_tipo_violencia_republica %>%
  ungroup() %>%
  filter(anyo == max(anyo)) %>%
  mutate(anyo = as.numeric(anyo) + 0.25)

colors <- c(
  "Robo" = "#B1B8BE",
  "Violencia familiar" = "#DE4212",
  "Lesiones" = "#B1B8BE",
  "Otros delitos del Fuero Común" = "#B1B8BE",
  "Daño a la propiedad" = "#B1B8BE",
  "Fraude" = "#B1B8BE" ,
  "Amenazas" = "#B1B8BE"
)
...
ggplot(data = ranking_tipo_violencia_republica, aes(anyo, rank, color = Tipo, group = Tipo, label = Tipo)) +
  ...
  theme(
    ...
    labs(x = "Año", y = "Rank", title = "Posición Violencia Familiar", subtitle = "Top 5 tipos de violencia en toda la Republica")
```
...
```{r fig.width=10}
violencia_tags_inicio <- ranking_tipo_violencia %>%
  ungroup() %>%
  filter(anyo == min(anyo)) %>%
  mutate(anyo = as.numeric(anyo) - 0.25)
violencia_tags_final <- ranking_tipo_violencia %>%
  ungroup() %>%
  filter(anyo == max(anyo)) %>%
  mutate(anyo = as.numeric(anyo) + 0.25)
...
ggplot(data = ranking_tipo_violencia, aes(anyo, rank, color = Tipo, group = Tipo, label = Tipo)) +
  ...
  theme(
    ...
    panel.grid.minor.x = element_blank(),
    panel.grid.major.x = element_line(color = 'grey60', linetype = 'dashed')) +
    labs(x = "Año", y = "Rank", title = "Posición Violencia Familiar", subtitle = "Top 5 tipos de violencia en Querétaro")
```
