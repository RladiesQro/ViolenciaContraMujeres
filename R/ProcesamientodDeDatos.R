...
DefinirMesesSinDatos <- function(datos_violencia) {
  datos_violencia %>%
    dplyr::group_by(fecha = lubridate::floor_date(fecha, unit = "month")) %>%
    dplyr::summarise(frecuencia_casos = sum(ocurrencia)) %>%
    dplyr::filter(frecuencia_casos == 0) %>%
    dplyr::pull(fecha)
}
...
AgregaPoligonos <- function(data_with_entity, poligonos_mx) {
  data_with_entity %>%
    ...
    dplyr::inner_join({
      poligonos_mx %>%
        dplyr::select(Entidad = nombre, geometry) %>%
        dplyr::mutate(Entidad = ifelse(Entidad == "Distrito Federal", "Ciudad de México", as.character(Entidad)))
    }, by = "Entidad") %>%
    sf::st_as_sf()
}
...
#'
#' @examples
#' \dontrun{AgregaTasaPoblacional(resumen_datos_estatal, poblacion_inegi_2015, columna_a_tasa = "casos_por_estado")}
AgregaTasaPoblacional <- function(resumen_datos_estatal, poblacion_inegi_2015, columna_a_tasa = "casos_por_estado") {
  resumen_datos_estatal %>%
    ...
    dplyr::mutate(tasa_100k = (get(columna_a_tasa) / Habitantes2015) * 100000) %>%
    dplyr::rename(poblacion_total = Habitantes2015)
}
